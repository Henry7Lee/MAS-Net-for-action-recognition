...
class MSM(nn.Module):
    ...
    def forward(self,d1,d2):
        ...
        d2 = d2.view(n_batch, self.num_segments, c, h, w).transpose(2,1).contiguous() #[n,c,t,h,w]

        d1_pool=nn.functional.avg_pool3d(d1, kernel_size=[self.num_segments, 1, 1])
        ...
        if n_batch !=1:
            d = self.bn_fc1(d)
            d = self.relu(d)
        d = self.conv_fc2(d) #[1, 64, 1, 56, 56]

        d = torch.unsqueeze(d, 1).view(-1, 2, c, 1, h, w)
...
class MAM(nn.Module):
    ...
    def forward(self, x):
        ...
        mapping0, last_frame = reshape_bottleneck_avg.split([self.num_segments - 1, 1], dim=1)  # n, t-1, c//r, 1, 1
        __, mapping1 = reshape_bottleneck_avg.split([1, self.num_segments - 1], dim=1)  # n, t-1, c//r, 1, 1

        avg_foward = (mapping0+0.5*mapping1)/1.5
        avg_foward_pad =torch.cat((avg_foward,last_frame),dim=1) #[n,t,1,h,w]
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        identity = x #[32, 64, 56, 56]

        out = self.conv1(x) #[32, 64, 56, 56]
        out = self.bn1(out)
        out = self.relu(out)

        Motion = self.MAM(out) #[32, 64, 56, 56]

        out = self.MSM(Motion,out) #[32, 64, 56, 56]
        out = self.TIM(out)

        out = self.conv2(out) #[32, 64, 56, 56]
        out = self.bn2(out)
        out = self.relu(out)
